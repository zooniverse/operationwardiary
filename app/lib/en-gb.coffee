...
If there was little to record or a major incident had taken place over a few days the dates might have been written as a range. In this case place the first tag at the start of the entry and enter the first date then place the second tag at the end of the entry and record the last date in the range.'
      time: 'Exact times were critically important to ensure that co-ordination between different units, between the artillery and infantry and between air and ground forces were achieved. Mistakes with time could prove fatal. Only tag entries in the Time column, recording them as hours and minutes using the 24 hour clock.  If a period of time is recorded as a range place the first tag at the start of the entry and enter the first time then place the second tag at the end and record the last time in the range. There is no need to tag times mentioned in the narrative.'
      place: "The Place column records the location of the unit on the date and at the time specified in columns 1 and 2. Other locations may be mentioned in the narrative but these can refer to a previous or future destination etc. It is important that we know the difference between locations in the Place column and those in the narrative so that other tags can be linked to a date, place and time correctly. When working down the Place column ensure that the 'Is the unit currently here?' box is ticked. Places in the narrative are often written in capital letters. When tagging them make sure the 'Is the unit currently here?' box is blank. Ignore locations that do not have a proper place name eg ignore Camp F or Area G. Sometimes locations are shown as a series of letters and numbers and there is a separate Grid Reference tag for these. Each unit started it's War Diary when it was mobilized, so the first entries may have been  recorded when they were still in the UK or embarking on a ship. This series of diaries covers the Western Front so all the other places mentioned should be locations in France or Belgium, sometimes called Flanders. When you enter a place name the map will show the location. If there is more than one place with the same name use the map to help you select the most likely location from the list. If the location seems incorrect or you are not sure select 'None of these/not sure'."
      person: '''
        Because the instructions for writing the diary stipulated that officers should be mentioned by name it has been assumed that very few other ranks can be identified in the diary entries. However units did take different approaches according to the conditions and circumstances they found themselves in and depending on the individual Adjutant. So now we want to tag every person mentioned by name and reveal why they were mentioned.
      '''
      person_fields: '''
                           <h4>Rank</h4>
  ...
  tools:
    ...
    person:
      author: "One officer in each unit was appointed as 'Adjutant', responsible for all the unit administration including writing the unit war diary. They may have signed each page or put their name on the last page of the month."
    ...
    domestic:
      accomodation: "Accommodation varied according to where the unit was in relation to the line. Tag terms including billets, quarters, bivouacs, camps, tents and dug outs."
